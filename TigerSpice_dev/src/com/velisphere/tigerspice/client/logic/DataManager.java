...
public class DataManager {
	...
	public void processP2L()
	{
		...
		Iterator<ConnectorSensorLogicCheck> it = canvas.getConnectorsSensorLogicCheck().iterator();
		...
		while (it.hasNext())
		{
			...
			// build empty action list object as no actions will be linked to this check
			...
			checkService.addNewCheck(current.getCheckUUID(), current.getSensor().getEndpointID(), current.getSensor().getPropertyID(), current.getCheckValue(), current.getOperator(), "unused", canvas.getUUID(), actions, new AsyncCallback<String>(){
				...
				public void onSuccess(String result) {
					...
					RootPanel.get().add(new HTML("Result from attempt to generate check and action P2L: " + result));
				}
	...
	{
		Iterator<ConnectorLogicCheckActor> it = canvas.getConnectorsLogicCheckActor().iterator();
		...
		while (it.hasNext())
		{
			...
			if(current.getLogicCheck().isAnd()) operator = "AND";
			if(current.getLogicCheck().isOr()) operator = "OR";
			...
			checkpathService.addNewMulticheck(current.getCheckUUID(), operator, "unused", canvas.getUUID(), actions, new AsyncCallback<String>(){
				...
				public void onSuccess(String result) {
					...
					RootPanel.get().add(new HTML("Result from attempt to generate check and action L2P: " + result));
				}
			...
			Iterator<ConnectorSensorLogicCheck> pit = current.getLogicCheck().getChildConnectors().iterator();
			
			while(pit.hasNext())
			{
				checkpathService.addNewMulticheckCheckLink(current.getCheckUUID(), pit.next().getCheckUUID(), canvas.getUUID(), new AsyncCallback<String>(){
					...
					public void onSuccess(String result) {
						// TODO Auto-generated method stub
						RootPanel.get().add(new HTML("Result from attempt to link checks and to multicheck: " + result));
					}
